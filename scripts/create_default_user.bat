...
psql -U postgres -c "INSERT INTO \"user\" VALUES ((select nextval('user_seq')), '%~1', 'NQg1uTRTu6Q7vvZA/j+OXDafTZw=', 'Default', 'User', now(), true, now(), '\x1cf09e162a9b3bbe20e7c0aaffec9e4d','t',now())" nics
psql -U postgres -c "INSERT INTO userorg VALUES ((select nextval('user_org_seq')), (select last_value from user_seq), '%~2', 0, now(), NULL, '', '', '')" nics
psql -U postgres -c "INSERT INTO userorg_workspace VALUES (%~3, (select last_value from user_org_seq), (select nextval('hibernate_sequence')), 't')" nics
psql -U postgres -c "INSERT INTO usersession values((select nextval('user_session_seq')), (select last_value from user_org_seq), 'default-session-id', now(), now())" nics
exit /B 0
